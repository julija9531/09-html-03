class Autocomplete {
  ...
  getMatches( text ) {
   ...
   //Получение полного списка слов:
    let list = [];
    for (let i = 0; i < Array.from(this.input.children).length; i++) {
      list.push({
        text: Array.from(this.input.children)[i].textContent,
        value: Array.from(this.input.children)[i].value,
      });
    }

    //Фильтраци по заданному значнию:
    let listFiltr = list.filter(item => item["text"].toLowerCase().includes(text.toLowerCase()))
    return listFiltr;
  }
}
